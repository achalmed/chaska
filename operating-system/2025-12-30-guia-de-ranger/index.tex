...
Ranger es un gestor de archivos de consola con: - Atajos de teclado
inspirados en Vim - Interfaz minimalista con curses - Vista de jerarquía
de directorios (multi-columna) - Previsualización automática de archivos
...
\textbf{Para funcionalidad completa:}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Determinar tipos de archivo}
\FunctionTok{sudo}\NormalTok{ apt{-}get install file}

\CommentTok{\# Detección mejorada de codificación}
\ExtensionTok{pip}\NormalTok{ install chardet}

\CommentTok{\# Nombres bidireccionales (árabe, hebreo)}
\ExtensionTok{pip}\NormalTok{ install python{-}bidi}

\CommentTok{\# Previsualización de imágenes}
\FunctionTok{sudo}\NormalTok{ apt{-}get install caca{-}utils w3m{-}img}

\CommentTok{\# Conversión de imágenes}
\FunctionTok{sudo}\NormalTok{ apt{-}get install imagemagick}

\CommentTok{\# Resaltado de sintaxis}
\FunctionTok{sudo}\NormalTok{ apt{-}get install highlight}
\CommentTok{\# O}
\ExtensionTok{pip}\NormalTok{ install pygments}

\CommentTok{\# Previsualización de archivos comprimidos}
\FunctionTok{sudo}\NormalTok{ apt{-}get install atool unrar p7zip{-}full}

\CommentTok{\# Previsualización de PDFs}
\FunctionTok{sudo}\NormalTok{ apt{-}get install poppler{-}utils}

\CommentTok{\# Previsualización de videos}
\FunctionTok{sudo}\NormalTok{ apt{-}get install ffmpegthumbnailer}

\CommentTok{\# Información de medios}
\FunctionTok{sudo}\NormalTok{ apt{-}get install mediainfo}
...
\subsection{Columnas Explicadas}\label{columnas-explicadas}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{gh} & Ir a HOME (\textasciitilde) \\
\texttt{ge} & Ir a /etc \\
\texttt{gu} & Ir a /usr \\
\texttt{gd} & Ir a /dev \\
\texttt{go} & Ir a /opt \\
...
\textbf{Ejemplo:} - \texttt{ft} - Salta al primer archivo que empiece
con `t' - \texttt{Ft} - Salta a archivos que empiecen con `t', presiona
...
\CommentTok{\# Navega a otro lugar}
...
\subsection{1. Preview de Archivos con
...
\subsection{2. Thumbnails de Videos}\label{thumbnails-de-videos}
...
\subsection{3. Preview de Imágenes (Terminal
...
\subsection{4. Búsqueda con FZF}\label{buxfasqueda-con-fzf}
...
\subsection{5. Integración con Git}\label{integraciuxf3n-con-git}
...
\subsection{6. Archivos Grandes}\label{archivos-grandes}
...
\subsection{7. Abrir en Programa
Externo}\label{abrir-en-programa-externo}
...
\subsection{9. Operaciones Bulk}\label{operaciones-bulk}
...
\subsection{10. Preview de Archivos
...
\textbf{Soluciones:} 1. Limitar tamaño de preview 2. Desactivar preview
de imágenes en directorios grandes 3. Reducir historial
