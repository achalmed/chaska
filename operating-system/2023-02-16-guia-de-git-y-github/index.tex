...
\title{Guía completa de Git y GitHub}


\shorttitle{Guía completa de Git y GitHub}
...
\abstract{Esta guía completa de Git y GitHub ofrece una visión integral
y actualizada del sistema de control de versiones más utilizado en el
desarrollo de software. Desde los fundamentos teóricos (arquitectura,
objetos y estados de archivos) hasta comandos esenciales, flujos de
trabajo diarios reales, resolución avanzada de conflictos, mejores
prácticas modernas (incluyendo Conventional Commits y trunk-based
development) y ejemplos prácticos aplicados en 2025, este documento
sirve como referencia exhaustiva tanto para principiantes que dan sus
primeros pasos como para desarrolladores intermedios que desean
optimizar su flujo de trabajo diario en entornos individuales, equipos
pequeños y proyectos colaborativos en GitHub. }
...
\hypersetup{
  pdftitle={Gu\'{\i}a completa de Git y GitHub},
  pdfauthor={Autor}
}
\makeatletter
...
\tableofcontents
\newpage
\section[Introduction]{Guía completa de Git y GitHub}
...
\tightlist
\item
  Sistema distribuido (cada desarrollador tiene una copia completa del
  historial)
\item
...
\textbf{Tres áreas principales:}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Working Directory}: Tu directorio de trabajo actual
\item
  \textbf{Staging Area (Index)}: Área de preparación para el próximo
  commit
\item
  \textbf{Repository (.git directory)}: Base de datos de objetos y
  metadata
\end{enumerate}
...
\FunctionTok{sudo}\NormalTok{ apt install libz{-}dev libssl{-}dev libcurl4{-}gnutls{-}dev }\DataTypeTok{\textbackslash{}}
\NormalTok{libexpat1{-}dev gettext cmake gcc}
\end{Highlighting}
\end{Shaded}
\item
...
\ExtensionTok{curl} \AttributeTok{{-}o}\NormalTok{ git.tar.gz }\DataTypeTok{\textbackslash{}}
\NormalTok{https://mirrors.edge.kernel.org/pub/software/scm/git/git{-}2.34.1.tar.gz}
...
\textbf{Buenas prácticas para mensajes:}
...
\textbf{Placeholders útiles:}

\begin{itemize}
\tightlist
\item
  \texttt{\%h}: Hash abreviado
\item
  \texttt{\%H}: Hash completo
\item
  \texttt{\%an}: Nombre del autor
\item
  \texttt{\%ae}: Email del autor
\item
  \texttt{\%ad}: Fecha del autor
\item
  \texttt{\%ar}: Fecha relativa
\item
  \texttt{\%s}: Mensaje del commit
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Eliminar scripts obsoletos}
\FunctionTok{git}\NormalTok{ rm sync{-}repos.py}
\FunctionTok{git}\NormalTok{ rm sync{-}repos.sh quick{-}sync.sh repo{-}status.sh}

\CommentTok{\# Quitar .quarto/ del índice sin borrarla del disco}
\CommentTok{\# (típico cuando .quarto/ ya estaba trackeada antes de añadirla a .gitignore)}
\FunctionTok{git}\NormalTok{ rm }\AttributeTok{{-}r} \AttributeTok{{-}{-}cached}\NormalTok{ .quarto/}
\FunctionTok{git}\NormalTok{ rm }\AttributeTok{{-}r} \AttributeTok{{-}{-}cached}\NormalTok{ site\_libs/}
\FunctionTok{git}\NormalTok{ rm }\AttributeTok{{-}r} \AttributeTok{{-}{-}cached}\NormalTok{ \_freeze/}
\FunctionTok{git}\NormalTok{ rm }\AttributeTok{{-}r} \AttributeTok{{-}{-}cached}\NormalTok{ \_site/}

\FunctionTok{cat}\NormalTok{ .gitignore }\KeywordTok{|} \FunctionTok{grep}\NormalTok{ site\_libs}

\FunctionTok{git}\NormalTok{ commit }\AttributeTok{{-}m} \StringTok{"chore: excluir carpetas generadas del índice desde git tracking"}
\FunctionTok{git}\NormalTok{ commit }\AttributeTok{{-}m} \StringTok{"chore: exclude site\_libs from git tracking"}

\CommentTok{\# Quitar un archivo de salida que se coló al tracking}
\FunctionTok{git}\NormalTok{ rm }\AttributeTok{{-}{-}cached}\NormalTok{ data/resultado\_intermedio.rds}
\FunctionTok{git}\NormalTok{ rm }\AttributeTok{{-}{-}cached}\NormalTok{ .RData}
\FunctionTok{git}\NormalTok{ commit }\AttributeTok{{-}m} \StringTok{"chore: exclude RData files from git tracking"}

\CommentTok{\# Renombrar}
\FunctionTok{git}\NormalTok{ mv repo{-}status.sh status.sh }
\FunctionTok{git}\NormalTok{ mv guia\_quarto\_borrador.md guia\_quarto.md}
\FunctionTok{git}\NormalTok{ mv referencias.bib bibliography.bib}

\CommentTok{\# Mover}
\FunctionTok{git}\NormalTok{ mv repos{-}config.yml config/repos{-}config.yml }
\FunctionTok{git}\NormalTok{ mv posts/2023{-}articulo{-}viejo.qmd posts/\_archivo/2023{-}articulo{-}viejo.qmd}
\end{Highlighting}
\end{Shaded}
...
\textbf{Tipos de merge:}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Fast-forward}: Avance directo sin commit de merge
\item
  \textbf{Three-way merge}: Crea commit de merge con dos padres
\item
  \textbf{Squash merge}: Combina todos los commits en uno
\end{enumerate}
...
\textbf{Rebase interactivo - comandos:}

\begin{itemize}
\tightlist
\item
  \texttt{pick}: Usar commit
\item
  \texttt{reword}: Cambiar mensaje
\item
  \texttt{edit}: Editar commit
\item
  \texttt{squash}: Fusionar con commit anterior
\item
  \texttt{fixup}: Como squash pero descarta mensaje
\item
  \texttt{drop}: Eliminar commit
\end{itemize}
...
\item
  \texttt{git\ add\ \textless{}archivo\textgreater{}} para marcar como
  resuelto
\item
  \texttt{git\ commit} o \texttt{git\ rebase\ -\/-continue}
\end{enumerate}
...
\subsection{Flujo individual / Freelance / Proyectos
personales}\label{flujo-individual-freelance-proyectos-personales}
...
\subsection{Flujo típico de equipo mediano/pequeño con Pull
Requests}\label{flujo-tuxedpico-de-equipo-medianopequeuxf1o-con-pull-requests}
...
\CommentTok{\#     Merge squash / merge commit / }
\CommentTok{\#     rebase \& merge (depende de la política del equipo)}
...
\subsection{Corrección rápida de bug en
producción}\label{correcciuxf3n-ruxe1pida-de-bug-en-producciuxf3n}
...
\subsection{Flujo cuando trabajas en varias tareas al mismo
tiempo}\label{flujo-cuando-trabajas-en-varias-tareas-al-mismo-tiempo}
...
\subsection{Mini-resumen de comandos que más se usan en la práctica
diaria}\label{mini-resumen-de-comandos-que-muxe1s-se-usan-en-la-pruxe1ctica-diaria}
...
\tightlist
\item
  \texttt{pre-commit}: Antes de crear commit
\item
  \texttt{prepare-commit-msg}: Antes de abrir editor de commit
\item
  \texttt{commit-msg}: Validar mensaje de commit
\item
  \texttt{post-commit}: Después de crear commit
\item
  \texttt{pre-push}: Antes de hacer push
\end{itemize}
...
\tightlist
\item
  \texttt{pre-receive}: Antes de aceptar push
\item
  \texttt{update}: Por cada rama actualizada
\item
  \texttt{post-receive}: Después de aceptar push
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{git}\NormalTok{ config }\AttributeTok{{-}{-}global}\NormalTok{ alias.lg }\StringTok{"log {-}{-}graph {-}{-}pretty=format:\textquotesingle{}\%Cred\%h\%Creset }\DataTypeTok{\textbackslash{}}
\StringTok{{-}\%C(yellow)\%d\%Creset \%s \%Cgreen(\%cr) \%C(bold blue)\textless{}\%an\textgreater{}\%Creset\textquotesingle{} {-}{-}abbrev{-}commit"}
...
\tightlist
\item
  \texttt{feat}: Nueva funcionalidad
\item
  \texttt{fix}: Corrección de bug
\item
  \texttt{docs}: Documentación
\item
  \texttt{style}: Formato (no afecta código)
\item
  \texttt{refactor}: Refactorización
\item
  \texttt{test}: Tests
\item
  \texttt{chore}: Tareas de mantenimiento
\item
...
\item
  ...
  actual.
\item
  \texttt{git\ status} - Siempre saber dónde estás
\item
  \texttt{git\ add} - Preparar cambios
\item
  \texttt{git\ commit\ -m\ "Primer\ commit"} - Guardar cambios
\item
  \texttt{git\ branch\ -M\ main}: Muestra las ramas existentes.
\item
  \texttt{git\ push\ -u\ origin\ main} - Compartir cambios
\item
  \texttt{git\ pull} - Obtener cambios
\item
  \texttt{git\ log} - Ver historial de commits
\item
  \texttt{git\ diff} - Ver diferencias
\item
